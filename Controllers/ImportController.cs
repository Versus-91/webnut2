...
namespace DataSystem.Controllers
{
    ...
    public class ImportController : Controller
    {
       ...
       private IHostingEnvironment hostingEnv;
        ...
        {           
          _context.TempFacility.FromSql("exec dbo.DeleteTempFaciclity").FirstOrDefaultAsync();
        }

        public void UpdateTempFacilityToMain()
        {           
          _context.FacilityInfo.FromSql("exec dbo.UpdateFacilityInfo").FirstOrDefaultAsync();
        }
        ...
        private IList<TempFacility> ExportDataFromExcelSheet(IWorksheet sheet, int startRowIndex, int startColumnIndex, int lastRowIndex)
        {
            ...
            for (int r = startRowIndex; r <= lastRowIndex; r++)
            {
                ...
                if (sheet[r, startColumnIndex + 10].HasNumber)
                    Facility.Lat = Convert.ToDecimal(sheet[r, startColumnIndex + 10].Number);
                if (sheet[r, startColumnIndex + 11].HasNumber)
                    Facility.Lon = Convert.ToDecimal(sheet[r, startColumnIndex + 11].Number);
